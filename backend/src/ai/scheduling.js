  ...
  try {
    ...
    for (const message of userMessages) {
      await openai.beta.threads.messages.create(thread.id, {
        role: "user",
        content: `[${dateTimeString}] ${message}`,
    ...
    const run = await openai.beta.threads.runs.create(thread.id, {
      ...
      additional_instructions: `The current date and time is ${dateTimeString}. Note that this conversation may span multiple days - always consider the timestamp of each message when determining context. When responding to messages, always consider the current date and time. If a message is from a previous day, treat it as a new conversation unless explicitly related to the current conversation.`,
    });
